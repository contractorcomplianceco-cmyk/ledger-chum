...
**Short answer up front:** **Option C — Shared CCA Financial Platform**, delivered *through* an API contract that ServiceConnect (and later Tara OS, QualifierConnect, Command Center) consumes. Option B is the correct interim state; Option A is not recommended.

**Critical framing:** Ledger today is a **UI Design Lab**. Every service, table, chart of accounts, and journal is mock data (`src/lib/mock/*`, `src/lib/api/services/*` returning `mockGet` / `mockMutation`). No database, no server functions, no authentication, no posting engine exists yet. This audit distinguishes *what is designed* from *what is built*.
...
### Non-standard capabilities Ledger *does* design richly
...
**ServiceConnect's existing billing / AR / payment UIs:** keep them. They call Ledger APIs. Data moves *to* Ledger for posting; results (invoice status, balance) flow back.
...
### Phase 1 — Read-only financial connection *(4–6 weeks)*
...
### Phase 2 — Invoice synchronization *(4–6 weeks)*
...
### Phase 3 — Payment synchronization *(3–5 weeks)*
...
### Phase 4 — Inventory / job-cost integration *(6–8 weeks)*
...
### Phase 5 — Full accounting workflow *(8–10 weeks)*
...
### Phase 6 — Rose OS financial intelligence *(ongoing)*
