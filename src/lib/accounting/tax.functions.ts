...
export const listJurisdictions = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: rows, error } = await context.supabase
      .from("tax_jurisdictions").select("*").eq("org_id", data.orgId).order("code");
...
export const upsertJurisdiction = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const q = data.id
      ? context.supabase.from("tax_jurisdictions").update(payload).eq("id", data.id).select().single()
...
export const listTaxCategories = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: rows, error } = await context.supabase
      .from("tax_categories").select("*").eq("org_id", data.orgId).order("code");
    if (error) throw new Error(error.message);
    return rows ?? [];
  });
...
export const upsertTaxCategory = createServerFn({ method: "POST" })
  ...
  .inputValidator((v) =>
    z.object({
      ...
      code: z.string().min(1).max(64),
      name: z.string().min(1).max(200),
      kind: z.enum(["sales","use","vat","gst","withholding","payroll","excise","other"]).default("sales"),
      isActive: z.boolean().default(true),
    }).parse(v),
  )
  .handler(async ({ data, context }) => {
    const payload = {
      org_id: data.orgId, code: data.code, name: data.name, kind: data.kind, is_active: data.isActive,
    };
    const q = data.id
...
export const listTaxRates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: rows, error } = await context.supabase
      .from("tax_rates").select("*").eq("org_id", data.orgId).order("effective_from", { ascending: false });
    if (error) throw new Error(error.message);
    return rows ?? [];
  });
...
export const listTaxLiabilities = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    let q = context.supabase
      .from("tax_liabilities").select("*").eq("org_id", data.orgId)
