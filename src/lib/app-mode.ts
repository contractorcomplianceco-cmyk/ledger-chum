 ...
 *
 * Set `VITE_APP_MODE=production` to run against the live backend. Anything else
 * (including the default) resolves to `demo`, keeping the Design Lab and local
 * previews fully self-contained with no network or secrets required.
 *
 * This module is client-safe: it only reads a build-time Vite env var and never
 * imports server-only code. Server code can read the same value from
 * `process.env.APP_MODE` via {@link resolveServerMode}.
 */
