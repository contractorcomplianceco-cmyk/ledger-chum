...
import { getApAging } from "@/lib/accounting/aging.functions";

export const Route = createFileRoute("/accounts-payable/aging")({
  head: () => ({
    meta: [
...
function ApAgingPage() {
  ...
  const fn = useServerFn(getApAging);
  const q = useQuery({
                ...
                <tbody>
                  {q.data.rows.map((r) => (
                    <tr key={r.vendorId} className="border-b last:border-0">
                      <td className="py-2 pr-3 font-medium">{r.vendorName}</td>
                      {BUCKETS.map((b) => (
