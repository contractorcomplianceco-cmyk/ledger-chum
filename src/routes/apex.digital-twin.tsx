...
export const Route = createFileRoute("/apex/digital-twin")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Model business decisions before making them. Demonstration scenarios only.",
      },
    ],
  }),
  component: TwinIndex,
      ...
      <ApexSection title="Baseline (current plan)">
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <ExperienceStat
            ...
            value={currency(BASELINE.trueAvailableCash)}
            tone="info"
          />
          ...
          <ExperienceStat
            label="Runway"
            value={`${BASELINE.runwayMonths} mo`}
            hint="Hiring capacity: 4"
          />
          <ExperienceStat
            label="Company Health"
            value={`${BASELINE.companyHealth}`}
            tone="success"
          />
          <ExperienceStat
            label="Financial Confidence"
            value={`${BASELINE.financialConfidence}%`}
            tone="info"
          />
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-2 text-[11px]">
                    ...
                    <div>
                      ...
                      <div className="font-bold tabular-nums text-foreground">
                        {s.outputs12mo.runwayMonths} mo
                      </div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">Health</div>
                      <div className="font-bold tabular-nums text-foreground">
                        {s.outputs12mo.companyHealth}
                      </div>
                    </div>
                  </div>
                </Card>
          ...
          <Card className="border-border/70 p-4 text-[12.5px]">
            <p className="text-muted-foreground">
              <strong className="text-foreground">
                Demonstration scenario — not a production forecast.
              </strong>{" "}
