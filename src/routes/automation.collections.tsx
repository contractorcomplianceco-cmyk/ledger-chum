...
function CollectionsPage() {
  ...
  const expectedThisWeek = COLLECTIONS.filter((c) => c.probability >= 70).reduce(
    (s, c) => s + c.amount,
    0,
  );
  ...
  return (
    ...
    >
      <section className="grid gap-3 sm:grid-cols-3">
        ...
        <Kpi
          label="At risk (< 50% prob)"
          value={currency(
            COLLECTIONS.filter((c) => c.probability < 50).reduce((s, c) => s + c.amount, 0),
          )}
          tone="destructive"
        />
      </section>
            ...
            <div className="grid gap-3 lg:grid-cols-[1.4fr_1fr_1fr_1fr_auto] lg:items-center">
              <div>
                ...
                <div className="mt-0.5 text-[11.5px] text-muted-foreground">
                  Behavior · {c.paymentBehavior.replace("_", " ")}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-0.5 font-tabular text-[11px]">
                  {c.probability}% · {c.expectedCollection}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <Button size="sm" variant="outline" className="h-7">
                  <ShieldAlert className="mr-1 h-3 w-3" /> Hold
                </Button>
