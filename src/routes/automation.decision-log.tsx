...
const TYPE_META: Record<DecisionEntry["type"], { label: string; tone: string }> = {
  ...
  pricing: {
    ...
    tone: "border-violet-400/30 bg-violet-500/10 text-violet-400",
  },
  ...
  software_cancel: {
    label: "Software cancel",
    tone: "border-cyan-400/30 bg-cyan-500/10 text-cyan-400",
  },
  campaign_increase: {
    label: "Campaign increase",
    tone: "border-emerald-400/30 bg-emerald-500/10 text-emerald-400",
  },
  vendor_reneg: {
    label: "Vendor negotiation",
    tone: "border-amber-400/30 bg-amber-500/10 text-amber-400",
  },
  reserve_change: { label: "Reserve change", tone: "border-brand/30 bg-brand/10 text-brand" },
  client_exception: {
    label: "Client exception",
    tone: "border-fuchsia-400/30 bg-fuchsia-500/10 text-fuchsia-400",
  },
};
                  ...
                  <div>
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {d.linkedRecords.map((r) => (
                        <span
                          key={r}
                          className="rounded-md border border-border/70 bg-muted/30 px-1.5 py-0.5 text-[10.5px] font-mono text-muted-foreground"
                        >
                          {r}
                        </span>
                      ))}
