  ...
  return (
    ...
    >
      <section className="grid gap-3 sm:grid-cols-4">
        ...
        <Metric
          label="High urgency"
          value={String(EXCEPTIONS.filter((e) => e.urgency === "high").length)}
          tone="destructive"
        />
        <Metric
          label="Cash risk"
          value={String(EXCEPTIONS.filter((e) => e.cashRisk).length)}
          tone="warning"
        />
        <Metric
          label="Compliance risk"
          value={String(EXCEPTIONS.filter((e) => e.compliance).length)}
          tone="warning"
        />
      </section>
        ...
        <Card className="border-border/70 p-0">
          ...
          {sorted.map((e) => (
            ...
            >
              ...
              <span className="font-tabular text-[12px] text-destructive">
                {e.dollarImpact ? currency(e.dollarImpact) : "—"}
              </span>
            </button>
          ))}
        </Card>
        ...
        {selected && (
          <Card className="border-border/70 p-5">
            <div className="text-[10.5px] font-semibold uppercase tracking-wide text-muted-foreground">
              {selected.id} · {EXCEPTION_TYPE_META[selected.type]}
            </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-1.5">
              <button className="rounded-md border border-border bg-surface px-2 py-1.5 text-[11.5px] hover:border-foreground/20">
                Assign
              </button>
              <button className="rounded-md border border-border bg-surface px-2 py-1.5 text-[11.5px] hover:border-foreground/20">
                Resolve
