...
function Approvals() {
  ...
  useEffect(() => {
    api.compensationOps.listApprovals().then(setRows);
  }, []);

  const filtered = useMemo(() => {
    if (tab === "awaiting") return rows.filter((r) => r.status === "awaiting");
    if (tab === "approved")
      return rows.filter((r) => r.status === "approved" || r.status === "approved_with_conditions");
        ...
        {filtered.map((a) => (
          <Card key={a.id} className="p-4 space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              {a.planName} · {a.compensationClass}
            </div>
            <div className="grid grid-cols-3 gap-2 text-sm">
              ...
              <div>
                ...
                <div className="font-tabular">{(a.marginImpact * 100).toFixed(1)}%</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Reserve</div>
                <div className="font-tabular">{currency(a.reserveImpact)}</div>
              </div>
            </div>
            <div className="text-xs">
              Recommendation: <span className="italic">{a.recommendation}</span>
            </div>
            ...
            {a.status === "awaiting" && (
              <div className="flex flex-wrap gap-1.5">
                <Button size="sm" onClick={() => showDemoToast("Approved")}>
                  Approve
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => showDemoToast("Approved with conditions")}
                >
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => showDemoToast("Changes requested")}
                >
                  Request changes
                </Button>
                <Button size="sm" variant="ghost" onClick={() => showDemoToast("Held")}>
                  Hold
                </Button>
                <Button size="sm" variant="ghost" onClick={() => showDemoToast("Rejected")}>
                  Reject
                </Button>
                <Button size="sm" variant="ghost" onClick={() => showDemoToast("Escalated")}>
                  Escalate
                </Button>
              </div>
            )}
