            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-3 text-xs">
                  ...
                  <span>
                    <span className="text-muted-foreground">Evidence:</span> {c.evidenceCount}
                  </span>
                  <span>
                    <span className="text-muted-foreground">Impact:</span>{" "}
                    {currency(c.financialImpact)}
                  </span>
                  <span>
                    <span className="text-muted-foreground">Approver:</span> {c.requiredApprover}
                  </span>
                </div>
                ...
                <div className="mt-3">
                  ...
                  <ul className="ml-4 mt-1 list-disc space-y-0.5 text-xs">
                    {c.timeline.map((t, i) => (
                      <li key={i}>
                        {t.at.slice(0, 10)} — {t.actor}: {t.action}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="flex flex-col gap-1.5">
                <Button
                  size="sm"
                  onClick={() => {
                    api.compensation.resolveConflict(c.id, "resolved");
                    showDemoToast("Conflict resolved");
                  }}
                >
                  Resolve
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => showDemoToast("Requested evidence")}
                >
                  Request evidence
                </Button>
                <Button size="sm" variant="outline" onClick={() => showDemoToast("Escalated")}>
                  Escalate
                </Button>
              </div>
            </div>
