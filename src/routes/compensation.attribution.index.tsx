      ...
      actions={
        <>
          <Button size="sm" variant="outline" asChild>
            <Link to="/compensation/attribution/conflicts">Conflicts</Link>
          </Button>
          <Button size="sm" variant="outline" asChild>
            <Link to="/compensation/attribution/evidence">Evidence</Link>
          </Button>
        </>
      }
    >
              ...
              <TableRow key={a.id}>
                ...
                <TableCell className="text-xs">
                  {a.contributions.map((c) => (
                    <div key={c.id}>
                      {c.participantName} <span className="text-muted-foreground">· {c.role}</span>{" "}
                ...
                <TableCell>
                  ...
                  >
                    {a.overallStatus.replaceAll("_", " ")}
                  </Badge>
                </TableCell>
                <TableCell className="text-right">
                  <Button size="sm" variant="ghost" asChild>
                    <Link to="/compensation/attribution/$id" params={{ id: a.id }}>
                      Open
                    </Link>
                  </Button>
                </TableCell>
      ...
      <Card className="p-5">
        ...
        <ul className="ml-4 list-disc space-y-1 text-sm">
          ...
          <li>
            Missing attribution, duplicate participants, and preexisting-pipeline conflicts must be
            flagged.
          </li>
        </ul>
      </Card>
