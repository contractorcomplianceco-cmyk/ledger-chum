      ...
      actions={
        <div className="flex gap-2">
          <Button size="sm" variant="outline" asChild>
            <Link to="/compensation/calculations/$id/preview" params={{ id }}>
              Preview
            </Link>
          </Button>
          <Button size="sm" disabled={readOnly} onClick={() => showDemoToast("Approved")}>
            Approve
          </Button>
        </div>
      }
      ...
      <Tabs defaultValue="summary">
        ...
        <TabsContent value="source">
          <Card className="p-5 text-sm space-y-2">
            ...
            <Field
              ...
              value={calc.source.collectionStatus.replace(/_/g, " ")}
            />
            ...
            <Field
              label="Refunds / Credits / Chargebacks"
              value={`${currency(calc.source.refunds)} / ${currency(calc.source.credits)} / ${currency(calc.source.chargebacks)}`}
            />
          </Card>
        </TabsContent>
              ...
              <TableBody>
                {calc.lines.map((l) => (
                  <TableRow key={l.id}>
                    ...
                    <TableCell className="text-right font-tabular">
                      {currency(l.baseAmount)}
                    </TableCell>
                    <TableCell className="text-xs">
                      {l.rate ? `${(l.rate * 100).toFixed(1)}%` : "—"}
                    </TableCell>
                    <TableCell className="text-right font-tabular">
                      {currency(l.grossAmount)}
                    </TableCell>
                    <TableCell className="text-right font-tabular text-muted-foreground">
                      {currency(l.holdback)}
                    </TableCell>
                    <TableCell className="text-right font-tabular">
                      {currency(l.adjustment)}
                    </TableCell>
                    ...
                    <TableCell className="text-xs">
                      {l.glExpenseAccount}
                      <div className="text-muted-foreground">{l.glPayableAccount}</div>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
        ...
        <TabsContent value="verification">
          <Card className="p-5 text-sm text-muted-foreground">
            See{" "}
            <Link to="/compensation/verification" className="underline">
              Verification queue
            </Link>{" "}
            for review status.
          </Card>
        ...
        <TabsContent value="approvals">
          <Card className="p-5 text-sm">
            Approval route: {calc.approvalRoute.join(" → ")}. See{" "}
            <Link to="/compensation/approvals" className="underline">
              Approval center
            </Link>
            .
          </Card>
        ...
        <TabsContent value="holdbacks">
          <Card className="p-5 text-sm">
            Total holdback:{" "}
            <span className="font-tabular font-semibold">{currency(calc.totalHoldback)}</span>. See{" "}
            <Link to="/compensation/holdbacks" className="underline">
              Holdback center
            </Link>
            .
          </Card>
        ...
        <TabsContent value="adjustments">
          <Card className="p-5 text-sm">
            Total adjustments:{" "}
            <span className="font-tabular font-semibold">{currency(calc.totalAdjustment)}</span>.
            See{" "}
            <Link to="/compensation/adjustments" className="underline">
              Adjustments
            </Link>
            .
          </Card>
                ...
                <TableBody>
                  {preview.entries.map((e, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell className="text-right font-tabular">
                        {e.debit ? currency(e.debit) : ""}
                      </TableCell>
                      <TableCell className="text-right font-tabular">
                        {e.credit ? currency(e.credit) : ""}
                      </TableCell>
