    ...
    >
      <Card className="p-4">
        <div className="flex flex-wrap items-center gap-2 text-xs">
          {STEPS.map((s, i) => (
            <button key={s.key} onClick={() => setStep(i)} className={`inline-flex items-center gap-1.5 rounded-md px-2 py-1 ${i === step ? "bg-brand/10 text-brand" : "text-muted-foreground"}`}>
      ...
      <Card className="space-y-3 p-6">
        <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Step {step + 1} of {STEPS.length}</div>
        ...
        {step === 1 && (
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div className="md:col-span-2 rounded-lg border border-success/40 bg-success/10 p-3 text-xs text-success">
              Payment collected and cleared — eligible to progress. Progression blocked for uncollected revenue unless plan is fixed / milestone.
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-2 text-sm">
            {[
              ...
              <div key={String(k)} className="flex items-center justify-between border-b border-border/40 py-2 last:border-0">
                <div>{String(k)} <span className="text-xs text-muted-foreground">{String(note)}</span></div>
        ...
        {step === 3 && (
          <div className="space-y-2 text-sm">
            <div className="rounded-lg border border-border/60 p-3">
              ...
              <div className="text-xs text-muted-foreground">Basis: collected_and_cleared_nrsr · Effective 2026-01-01 · House rule: suppress · Survival: 12 months</div>
              <Badge variant="outline" className="mt-2 border-success/40 bg-success/10 text-success">Invariants pass</Badge>
            </div>
            <div className="rounded-lg border border-border/60 p-3">
              <div className="font-semibold">Tara — Brand Ambassador (5%) — v2</div>
              <div className="text-xs text-muted-foreground">Carved from standard 10% pool. Stacking: elect per milestone. Legal review: cleared.</div>
              <Badge variant="outline" className="mt-2 border-success/40 bg-success/10 text-success">Attribution eligible</Badge>
            </div>
          </div>
        )}
          ...
          <div className="space-y-3 text-sm">
            <div className="rounded-lg border border-border/60 p-3">
              ...
              <div className="space-y-1 text-xs">
                <div className="flex justify-between"><span>Tara Casella — brand ambassador</span><span>50%</span></div>
                <div className="flex justify-between"><span>Jamie Rivera — closer</span><span>50%</span></div>
              </div>
            </div>
            <div className="rounded-lg border border-border/60 p-3">
              <div className="mb-2 flex items-center justify-between">
              ...
              <div className="text-xs text-muted-foreground">Not applied on this transaction — services only.</div>
            </div>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-2 text-sm">
            <div className="flex justify-between border-b border-border/40 py-1"><span>Base (realized)</span><span className="font-tabular">{currency(10800)}</span></div>
            <div className="flex justify-between border-b border-border/40 py-1"><span>Applied rate</span><span>10%</span></div>
            <div className="flex justify-between border-b border-border/40 py-1"><span>Pool amount</span><span className="font-tabular">{currency(1080)}</span></div>
            <div className="flex justify-between border-b border-border/40 py-1"><span>Tara (5%)</span><span className="font-tabular">{currency(540)}</span></div>
            <div className="flex justify-between border-b border-border/40 py-1"><span>Jamie (5%)</span><span className="font-tabular">{currency(540)}</span></div>
            <div className="flex justify-between border-b border-border/40 py-1"><span>Holdback (10%)</span><span className="font-tabular">-{currency(108)}</span></div>
            <div className="flex justify-between rounded-md bg-brand/5 p-2 font-semibold"><span>Net payable</span><span className="font-tabular">{currency(972)}</span></div>
          </div>
        )}
        ...
        {step === 7 && (
          <div className="space-y-2 text-xs">
            ...
            <div className="rounded-md border border-border/60 p-3">Calculation version 1 · Plan v3 · Rule version 2026.07</div>
          </div>
        )}
        ...
        {step === 8 && (
          <div className="rounded-lg border border-brand/40 bg-brand/5 p-4 text-sm">
            ...
            <p className="mt-2 text-foreground/90">
              CCA collected and cleared $12,000 from Meridian Advisors. After excluding $1,200 in pass-through fees, the realized commissionable revenue is $10,800.
              Tara qualifies for 5% through the Brand Ambassador pool, and the salesperson receives the remaining 5% of the standard 10% sales pool.
              Total compensation is $1,080; a 10% holdback ($108) is applied leaving $972 net payable.
            </p>
          </div>
        )}
        ...
        {step === 9 && (
          <div className="space-y-3 text-sm">
            <div>Required verification: attribution, pass-through, split validation, margin guardrail</div>
            ...
            <div className="flex gap-2">
              <Button onClick={() => showDemoToast("Calculation submitted for review")}>Submit</Button>
              <Button variant="outline" onClick={() => showDemoToast("Saved as draft")}>Save Draft</Button>
            </div>
          </div>
        )}
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <Button onClick={advance} disabled={step === STEPS.length - 1}>Next</Button>
        </div>
      </Card>
    </CompensationShell>
  );
