...
function Clawbacks() {
  ...
  useEffect(() => {
    api.compensationOps.listClawbacks().then(setRows);
  }, []);
  ...
  return (
    <CompensationShell
      ...
      actions={
        <Button size="sm" onClick={() => showDemoToast("Clawback drafted")}>
          New Clawback
        </Button>
      }
    >
          ...
          <Card key={c.id} className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
              <div>
                ...
                <div className="font-tabular font-semibold">{currency(c.grossClawback)}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Recovered</div>
                <div className="font-tabular">{currency(c.amountRecovered)}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Remaining</div>
                <div className="font-tabular font-semibold">{currency(c.remainingAmount)}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Method</div>
                <div className="text-xs">{c.recoveryMethod?.replace(/_/g, " ") ?? "—"}</div>
              </div>
            </div>
            <div className="text-xs text-muted-foreground">
              Accounting: {c.accountingImpact}
              <br />
                ...
                {c.recoveries.map((r) => (
                  <div key={r.id} className="flex justify-between">
                    ...
                    <span className="font-tabular">
                      {currency(r.amount)} · {new Date(r.recordedAt).toLocaleDateString()}
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              <Button size="sm" onClick={() => showDemoToast("Clawback approved")}>
                Approve
              </Button>
              <Button
                size="sm"
                variant="outline"
                onClick={() => showDemoToast("Recovery recorded")}
              >
                Record Recovery
              </Button>
              <Button size="sm" variant="ghost" onClick={() => showDemoToast("Written off")}>
                Write-off
              </Button>
              <Button size="sm" variant="ghost" onClick={() => showDemoToast("Sent to legal")}>
                Legal collection
              </Button>
            </div>
          </Card>
