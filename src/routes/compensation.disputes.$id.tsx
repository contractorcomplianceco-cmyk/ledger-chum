...
function DisputeDetail() {
  ...
  useEffect(() => { api.compensationOps.getDispute(id).then(setD); }, [id]);

  if (!d) return <CompensationShell title="Dispute"><Card className="p-6 text-sm text-muted-foreground">Loading…</Card></CompensationShell>;
    ...
    <CompensationShell
      ...
      actions={
        <div className="flex gap-2">
          <Button size="sm" onClick={() => showDemoToast("Dispute resolved")}>Resolve</Button>
          <Button size="sm" variant="outline" onClick={() => showDemoToast("Evidence added")}>Add evidence</Button>
        </div>
      }
    >
      <Card className="p-5 grid gap-3 md:grid-cols-3 text-sm">
        ...
        <div className="md:col-span-3">
          <div className="text-xs uppercase tracking-wide text-muted-foreground">Proposed resolution</div>
        ...
        <ul className="space-y-1 text-xs">
          {d.auditTimeline.map((e, i) => (
            <li key={i} className="flex justify-between border-b border-border/40 py-1">
              ...
              <span className="text-muted-foreground">{e.actor} · {new Date(e.at).toLocaleString()}</span>
            </li>
          ))}
        </ul>
      </Card>
