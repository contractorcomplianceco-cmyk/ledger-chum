...
function DisputesList() {
  ...
  useEffect(() => { api.compensationOps.listDisputes().then(setRows); }, []);
  ...
  return (
    <CompensationShell
      ...
      actions={<Button size="sm" onClick={() => showDemoToast("Dispute filed")}>File Dispute</Button>}
    >
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">Amount</TableHead><TableHead>Filed</TableHead>
              <TableHead>Reviewer</TableHead><TableHead>SLA</TableHead><TableHead>Status</TableHead><TableHead />
            </TableRow>
          </TableHeader>
          <TableBody>
            {rows.map((d) => (
              <TableRow key={d.id}>
                <TableCell className="font-mono text-xs"><Link to="/compensation/disputes/$id" params={{ id: d.id }} className="hover:underline">{d.id}</Link></TableCell>
                <TableCell>{d.participantName}</TableCell>
                <TableCell className="text-xs">{d.type.replace(/_/g, " ")}</TableCell>
                <TableCell className="text-right font-tabular">{currency(d.amountInDispute)}</TableCell>
                ...
                <TableCell><Badge variant="outline" className="text-[10px]">{d.status.replace(/_/g, " ")}</Badge></TableCell>
                <TableCell className="text-right"><Button size="sm" variant="ghost" asChild><Link to="/compensation/disputes/$id" params={{ id: d.id }}>Open</Link></Button></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
