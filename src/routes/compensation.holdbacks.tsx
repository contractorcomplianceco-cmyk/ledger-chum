...
function Holdbacks() {
  ...
  useEffect(() => {
    api.compensationOps.listHoldbacks().then(setRows);
  }, []);
            ...
            {rows.map((h) => (
              <TableRow key={h.id}>
                ...
                <TableCell className="text-right font-tabular">
                  {currency(h.originalCompensation)}
                </TableCell>
                <TableCell className="text-right text-xs">
                  {(h.holdbackPercent * 100).toFixed(0)}%
                </TableCell>
                <TableCell className="text-right font-tabular">
                  {currency(h.holdbackAmount)}
                </TableCell>
                <TableCell className="text-xs">{h.holdStart}</TableCell>
                <TableCell className="text-xs">{h.chargebackWindowEnd}</TableCell>
                <TableCell>
                  <Badge variant="outline" className={risk(h.riskStatus)}>
                    {h.riskStatus}
                  </Badge>
                </TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => showDemoToast("Released holdback")}
                      disabled={
                        h.status !== "eligible_for_release" &&
                        h.releaseMethod !== "manual" &&
                        h.riskStatus === "high"
                      }
                    >
                      Release
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => showDemoToast("Extended hold")}
                    >
                      Extend
                    </Button>
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => showDemoToast("Converted to clawback")}
                    >
                      Convert
                    </Button>
                  </div>
