  ...
  head: () => ({
    meta: [
      { title: "Compensation Operations — LedgerOS" },
      {
        name: "description",
        content:
          "Compensation operations dashboard — calculations, approvals, payables, holdbacks, disputes, reconciliation.",
      },
    ],
  }),
...
function OpsDashboard() {
  ...
  const projected = sum(
    calcs
      .filter((c) => c.status === "projected" || c.status === "pending_clearance")
      .map((c) => c.totalGross),
  );
  const calculated = sum(
    calcs
      .filter(
        (c) => c.status === "calculated" || c.status === "approved" || c.status === "reserved",
      )
      .map((c) => c.totalGross),
  );
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Every stage preserves policy snapshot, invariants, and audit trail.
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          <div className="mb-3 flex items-center justify-between">
            ...
            <Button asChild size="sm" variant="ghost">
              <Link to="/compensation/payables">View all</Link>
            </Button>
          </div>
            ...
            {payables.slice(0, 5).map((p) => (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {p.destination.replace(/_/g, " ")} · {p.scheduledDate}
                  </div>
                </div>
        ...
        <Card className="p-5">
          <div className="mb-3 flex items-center justify-between">
            ...
            <Button asChild size="sm" variant="ghost">
              <Link to="/compensation/approvals">Queue</Link>
            </Button>
          </div>
          <div className="space-y-2 text-sm">
            {approvals
            ...
            {holdbacks.slice(0, 5).map((h) => (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    Window ends {h.chargebackWindowEnd} · {h.status.replace(/_/g, " ")}
                  </div>
                </div>
        ...
        <Card className="p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="text-xs text-muted-foreground">Compensation reserve</div>
              <div className="font-tabular text-lg font-bold">{currency(reserved)}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">Available cash</div>
              <div className="font-tabular text-lg font-bold">{currency(451510)}</div>
            </div>
          </div>
          <div className="mt-3 text-xs text-muted-foreground">
            Reserve headroom looks healthy. See{" "}
            <Link to="/cash-availability" className="underline">
              Cash Availability
            </Link>{" "}
            for full waterfall.
          </div>
      ...
      <Card className="p-4">
        <div className="text-xs text-muted-foreground">
          Every calculated compensation excludes pass-through funds and uncollected revenue by
          default. Partial payments compute pro-rata. Stacked pools are shown separately — never
          blended.
        </div>
      </Card>
    </CompensationShell>
