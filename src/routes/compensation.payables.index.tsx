...
const STATUS_TABS: Array<"all" | PayableStatus> = [
  "all",
  "draft",
  "pending_approval",
  "approved",
...
function PayablesList() {
  ...
  useEffect(() => {
    api.compensationOps.listPayables().then((r) => setRows(r.data));
  }, []);
  ...
  return (
    <CompensationShell
      title="Payables"
      description="Payables represent net disbursement obligations. No real money moves — mock demonstrations only."
    >
      <Card className="p-3">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">Draw Offset</TableHead>
              <TableHead className="text-right">Clawback Offset</TableHead>
            ...
            {filtered.map((p) => (
              <TableRow key={p.id}>
                ...
                <TableCell className="text-right">
                  <Button size="sm" variant="ghost" onClick={() => showDemoToast("Approved")}>
                    Approve
                  </Button>
                </TableCell>
              </TableRow>
