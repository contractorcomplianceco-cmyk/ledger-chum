...
function BatchDetail() {
  ...
  useEffect(() => {
    api.compensationOps.getPaymentBatch(id).then(setB);
  }, [id]);

  if (!b)
    return (
      <CompensationShell title="Batch">
  ...
  return (
    <CompensationShell
      ...
      title={id}
      description={`${b.destination.replace(/_/g, " ")} · ${b.periodStart} → ${b.periodEnd}`}
      actions={
        <div className="flex gap-2">
          <Button size="sm" onClick={() => showDemoToast("Batch approved")}>
            Approve
          </Button>
          <Button size="sm" variant="outline" onClick={() => showDemoToast("Marked exported")}>
            Mark exported
          </Button>
          <Button size="sm" variant="ghost" onClick={() => showDemoToast("Reconciled batch")}>
            Reconcile
          </Button>
        </div>
      }
    >
      <Card className="p-4">
        <div className="flex flex-wrap gap-2 text-xs">
          {FLOW.map((s, i) => (
            <div
              key={s}
              className={`inline-flex items-center gap-1.5 rounded-md px-2 py-1 ${i <= currentIdx ? "bg-brand/10 text-brand" : "border border-border/40 text-muted-foreground"}`}
            >
              <span>{i + 1}.</span>
      ...
      <Card className="p-5 grid gap-3 md:grid-cols-4 text-sm">
        ...
        <F
          label="Net"
          value={<span className="font-semibold text-lg">{currency(b.netAmount)}</span>}
        />
