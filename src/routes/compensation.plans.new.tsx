...
const STEPS = [
  {
    key: "identity",
    title: "Plan Identity",
    desc: "Name, description, family, class, owner, effective dates, review flags",
  },
  {
    key: "basis",
    title: "Calculation Basis",
    desc: "Select from 15 supported bases (collected NRSR, gross profit, milestone, team pool, revenue share…)",
  },
  {
    key: "eligibility",
    title: "Eligibility",
    desc: "Participants, roles, departments, services, products, apps, channels, entities, customer types, geography, house-account behavior, renewal/expansion, event, strategic relationship",
  },
  {
    key: "rate",
    title: "Rate & Formula",
    desc: "Percentage, fixed, tiered, accelerator, decelerator, min/max, cap, floor, pool size, split method, margin floor, per-plan overrides",
  },
  {
    key: "deductions",
    title: "Deductions & Exclusions",
    desc: "Invariant defaults preserved: pass-through excluded, uncollected excluded, refunds/credits/chargebacks/discounts deducted, noncommissionable excluded. Relaxation requires legal + Owner.",
  },
  {
    key: "lifecycle",
    title: "Timing & Lifecycle",
    desc: "Payment-clearing lag, chargeback window, holdback %, holdback release, draw offset order, payable timing, payroll/AP destination, post-termination survival",
  },
  {
    key: "approval",
    title: "Approval Route",
    desc: "Verification owner, accounting reviewer, manager, Owner threshold, legal-review trigger, manual adjustment threshold, high-value threshold, margin-override threshold",
  },
  {
    key: "gl",
    title: "GL & Payment Mapping",
    desc: "Expense/payable/payroll/AP/reserve/holdback/clawback accounts, entity assignment",
  },
  {
    key: "summary",
    title: "Plain-Language Summary",
    desc: "Human-readable explanation shown to reviewers and participants",
  },
  {
    key: "preview",
    title: "Preview & Submit",
    desc: "Example calculation, margin/cash impact, invariant checks, resolved policy snapshot, required approvals",
  },
];
                ...
                <li key={s.key}>
                  ...
                  >
                    {done ? (
                      ...
                      <Circle
                        className={`mt-0.5 h-4 w-4 ${active ? "text-brand" : "text-muted-foreground"}`}
                      />
                    )}
                    <div>
        ...
        <Card className="p-6">
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input
                  ...
                  placeholder="Standard Sales Pool — 10% Collected"
                />
              </div>
              <div>
              ...
              <div className="flex flex-wrap gap-3 text-sm">
                <label className="flex items-center gap-2">
                  <input type="checkbox" defaultChecked /> Legal review required
                </label>
                <label className="flex items-center gap-2">
                  <input type="checkbox" defaultChecked /> Accounting review required
                </label>
                <label className="flex items-center gap-2">
          ...
          {step === 1 && (
            <div className="grid grid-cols-2 gap-2 text-sm">
              {BASES.map((b) => (
                <label
                  key={b}
                  className={`flex cursor-pointer items-center gap-2 rounded-md border p-2 ${basis === b ? "border-brand bg-brand/5" : "border-border"}`}
                >
          ...
          {step === 3 && (
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                <Label>Tiered rate table</Label>
                <Input placeholder="0-50k @ 8%; 50k+ @ 12%" />
              </div>
              <div>
                <Label>Accelerator</Label>
                <Input placeholder="×1.25 above 120% quota" />
              </div>
              <div>
                <Label>Decelerator</Label>
                <Input placeholder="×0.5 below 60% quota" />
              </div>
              <div>
              ...
              <div>
                ...
                <Input placeholder="10% of realized NRSR" />
              </div>
              <div>
                <Label>Split method</Label>
                <Input placeholder="Even, weighted, evidence-weighted…" />
              </div>
              <div>
          ...
          {step === 4 && (
            <div className="space-y-2 text-sm">
              <div className="rounded-md border border-warning/40 bg-warning/5 p-3">
                Invariant defaults apply: pass-through excluded, uncollected revenue excluded,
                refunds/credits/chargebacks/discounts deducted, noncommissionable items excluded.
                Relaxing these requires legal + Owner approval with a written reason.
              </div>
              {[
                "Pass-through",
                "Uncollected revenue",
          ...
          {step === 5 && (
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <Label>Payment-clearing lag (days)</Label>
                <Input defaultValue="3" />
              </div>
              <div>
                <Label>Chargeback window (days)</Label>
              ...
              <div>
                <Label>Payable timing</Label>
                <Input placeholder="Monthly, after clearing" />
              </div>
              <div>
          ...
          {step === 6 && (
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <Input placeholder="Accounting Lead" />
              </div>
              <div>
              ...
              <div>
                <Label>Legal-review trigger</Label>
                <Input placeholder="Investor, equity, sensitive" />
              </div>
              <div>
                <Label>Manual adjustment threshold</Label>
              ...
              <div>
                <Label>Margin-override threshold</Label>
            ...
            <div className="space-y-3 text-sm">
              <div className="rounded-md border border-border/60 p-3">
                ...
                <p className="text-muted-foreground">
                  $12,000 gross → $10,800 realized (10% pass-through excluded) → $1,080 pool → 50/50
                  split → $540 Tara / $540 salesperson.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-md border border-border/60 p-3">
          ...
          <div className="mt-6 flex items-center justify-between">
            ...
            {step < STEPS.length - 1 ? (
              ...
              <Button onClick={() => showDemoToast("Plan submitted for review")}>
                Submit for review
              </Button>
            )}
          </div>
