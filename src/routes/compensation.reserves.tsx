...
function Reserves() {
  ...
  useEffect(() => {
    api.compensationOps.listReserves().then(setRows);
  }, []);
  ...
  return (
    <CompensationShell
      title="Compensation Reserves"
      description="Reserves reflect approved-but-not-payable, holdbacks, and disputed amounts. Never blends with cash."
    >
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        />
        <KpiCard
          label="Reserve vs Available Cash"
          value={451510}
          delta={11.4}
          trend="up"
          icon={PiggyBank}
          ...
          <TableBody>
            {rows.map((r) => (
              <TableRow key={r.id}>
                ...
                <TableCell className="text-right font-tabular">
                  {currency(r.chargebackExposure)}
                </TableCell>
                <TableCell>
                  <Badge variant="outline" className="text-[10px]">
                    {r.destination.replace(/_/g, " ")}
                  </Badge>
                </TableCell>
