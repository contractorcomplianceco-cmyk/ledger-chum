...
function ExpensesDashboard() {
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      compareLabel: `${k.pendingCount} items`,
    },
    {
      label: "Reimbursable",
      value: k.reimbursable,
      icon: RefreshCw,
      tone: "cyan" as const,
      delta: 3.1,
      trend: "up" as const,
      to: "/expenses/reimbursements",
      compareLabel: "employee expenses",
    },
    {
      label: "Unmatched",
      value: k.unmatched,
      ...
      delta: -8.3,
      trend: "down" as const,
      to: "/expenses/matching",
      format: "number" as const,
      compareLabel: "need review",
    },
    {
      ...
      delta: -2.0,
      trend: "down" as const,
      to: "/expenses/receipts",
      format: "number" as const,
      compareLabel: "receipt required",
    },
    {
      ...
      format: "number" as const,
      compareLabel: "vs policy",
    },
    {
      label: "Subscription Spend",
      value: k.subscriptions,
      icon: Sparkles,
      tone: "cyan" as const,
      delta: 6.4,
      trend: "up" as const,
      to: "/expenses/subscriptions",
      compareLabel: "recurring",
    },
    {
      label: "Anomalies",
      value: k.anomalies,
      ...
      delta: 1,
      trend: "up" as const,
      to: "/expenses/intelligence",
      format: "number" as const,
      compareLabel: "AI flagged",
    },
  ];
    ...
    <div className="space-y-5">
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {kpiCards.map((c) => (
          <Link
            key={c.label}
            to={c.to as "/expenses"}
            className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand rounded-2xl"
          >
            <KpiCard {...c} />
          </Link>
        ))}
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        ...
        <Card className="border-border/70 p-4">
          ...
          <div className="mt-3 space-y-2">
            {SPEND_BY_CATEGORY.filter((c) => c.value > 0)
        ...
        <Card className="border-border/70 p-4">
          ...
          <div className="mt-2 space-y-1.5">
            {APPROVAL_QUEUE.slice(0, 5).map((e) => (
              <div
                key={e.id}
                className="flex items-center justify-between rounded-md border border-border/70 px-2.5 py-1.5 text-[12px]"
              >
        ...
        <Card className="border-border/70 p-4">
          ...
          <div className="mt-2 space-y-1.5">
            {ANOMALIES.slice(0, 3).map((a) => (
              <div
                key={a.id}
                className="rounded-md border border-border/70 px-2.5 py-1.5 text-[12px]"
              >
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        ...
        <Card className="border-border/70 p-4">
          ...
          <div className="mt-2 space-y-1.5">
            {RECOVERIES.filter(
              (r) => r.status === "eligible" || r.status === "on_draft" || r.status === "at_risk",
            ).map((r) => (
              <div
                key={r.id}
                className="flex items-center justify-between rounded-md border border-border/70 px-2.5 py-1.5 text-[12px]"
              >
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-muted-foreground">
                    {r.expense} · {r.contractRule}
                  </div>
                </div>
                <div className="text-right font-tabular font-semibold">
            ...
            {VENDOR_SPEND.slice(0, 6).map((v) => (
              <div key={v.vendor} className="rounded-md border border-border/70 p-3">
                ...
                <div className="mt-0.5 text-[11px] text-muted-foreground">
                  {v.department} · {v.subscription ? "Subscription" : "One-off"}
                </div>
              </div>
            ))}
          </div>
