...
const VIEWS: Array<{ id: string; label: string; test: (e: Expense) => boolean }> = [
  ...
  { id: "mine", label: "My Expenses", test: (e) => e.employee === "Rose Taylor" },
  {
    ...
    test: (e) => e.status.startsWith("pending") || e.status === "submitted",
  },
  ...
  { id: "reimbursable", label: "Reimbursable", test: (e) => e.reimbursable },
  {
    id: "unmatched",
    label: "Unmatched",
    test: (e) => e.match === "none" || e.match === "possible",
  },
  {
    id: "duplicate",
    label: "Possible Duplicate",
    test: (e) => e.match === "duplicate" || e.policy === "possible_duplicate",
  },
  ...
  { id: "anomaly", label: "Anomaly Detected", test: (e) => !!e.anomaly },
  {
    id: "subscription",
    label: "Subscription",
    test: (e) => ["technology", "ai"].includes(e.category),
  },
  {
    id: "ready",
    label: "Ready for Reimbursement",
    test: (e) => e.reimbursable && e.status === "approved" && e.reimbursement !== "paid",
  },
...
function ExpenseListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return EXPENSES.filter(v.test).filter((e) => {
      if (!q) return true;
      return `${e.vendor} ${e.employee} ${e.description} ${e.department} ${e.client ?? ""}`
        .toLowerCase()
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Search vendor, employee, note"
