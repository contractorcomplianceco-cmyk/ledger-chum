...
const STATUSES: Array<"all" | SubscriptionStatus> = [
  "all",
  "healthy",
  "underused",
  "price_increase",
...
function SubscriptionsPage() {
  ...
  const totalAnnual = SUBSCRIPTIONS.reduce(
    (a, s) => a + (s.frequency === "annual" ? s.currentCost : s.currentCost * 12),
    0,
  );
  const savings = SUBSCRIPTIONS.filter(
    (s) =>
      s.status === "underused" || s.status === "cancellation_candidate" || s.status === "no_owner",
    ...
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-3">
        <StatCard
          ...
          sub={`${SUBSCRIPTIONS.length} subscriptions`}
        />
        <StatCard
          label="Potential savings"
          value={currency(savings)}
          sub="cancel / right-size"
          tone="success"
        />
        <StatCard
          label="Needs attention"
          value={String(SUBSCRIPTIONS.filter((s) => s.status !== "healthy").length)}
