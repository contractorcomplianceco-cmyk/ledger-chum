...
import {
  Search, RefreshCw, CheckCircle2, AlertTriangle, Clock, XCircle, Beaker,
} from "lucide-react";
  ...
  const activityRows = useMemo(() => {
    if (live && eventsQ.data) {
      return eventsQ.data.map((e) => ({
        ...
        externalId: (e.after as { externalId?: string } | null)?.externalId
          ?? e.target_id?.slice(0, 8)
          ?? "—",
        customer: (e.after as { customer?: string; customerName?: string } | null)?.customer
          ?? (e.after as { customerName?: string } | null)?.customerName ?? "—",
        amount: Number((e.after as { total?: number; amount?: number } | null)?.total
          ?? (e.after as { amount?: number } | null)?.amount ?? 0),
        ...
        {tab === "Activity" && (
          <>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <FilterGroup value={evt} onChange={(v) => setEvt(v as typeof evt)} options={EVENT_FILTERS} />
              <FilterGroup value={status} onChange={(v) => setStatus(v as typeof status)} options={STATUS_FILTERS} />
            ...
            <Card className="overflow-hidden border-border/70 bg-surface shadow-card">
              ...
              <div className="divide-y divide-border/60">
                {filtered.map((e) => (
                  <div key={e.id} className="grid grid-cols-[130px_170px_140px_1fr_120px_130px_120px] items-center gap-3 px-4 py-3 text-[13px]">
                    ...
                    <div className="truncate font-mono text-[12px] text-foreground">{e.externalId}</div>
                    <div className="min-w-0">
                      <div className="truncate text-foreground">{e.customer}</div>
                      <div className="truncate text-[11px] text-muted-foreground">{e.syncResult}</div>
                    </div>
          ...
          <Card className="overflow-hidden border-border/70 bg-surface shadow-card">
            ...
            <div className="divide-y divide-border/60">
              {(live ? syncQ.data ?? [] : []).map((r) => (
                <div key={r.id} className="grid grid-cols-[1fr_140px_140px_120px_130px_100px] items-center gap-3 px-4 py-3 text-[13px]">
                  <div className="truncate font-mono text-foreground">{r.endpoint}</div>
                  <div className="truncate font-mono text-[12px] text-muted-foreground">{r.external_id ?? "—"}</div>
                  <div className="truncate font-mono text-[11px] text-muted-foreground">{r.idempotency_key.slice(0, 12)}…</div>
                  <div className="truncate text-muted-foreground">{r.source}</div>
                  <div className="text-[12px] text-muted-foreground">{fmtRelative(r.created_at)}</div>
                  <div><SyncBadge status={r.status === "success" ? "success" : r.status === "error" ? "failed" : "pending"} /></div>
                </div>
              ))}
              {(!live || (syncQ.data ?? []).length === 0) && (
                <div className="px-4 py-10 text-center text-[12px] text-muted-foreground">
                  {live ? "No sync records yet. Trigger a request via /api/public/integrations/*." : "Sign in to view live sync history."}
                </div>
              )}
            </div>
          </Card>
            ...
            {DEMO_SYSTEMS.map((s) => (
              <Card key={s.id} className="border-border/70 bg-surface p-4 shadow-card">
                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  ...
                  <MiniStat label="Failed 24h" value={s.failedCount24h.toString()} tone={s.failedCount24h > 0 ? "warn" : "muted"} />
                  <MiniStat label="Pending" value={s.pendingCount.toString()} tone={s.pendingCount > 0 ? "warn" : "muted"} />
                </div>
                ...
                {s.retryStatus && (
                  <div className="mt-1 flex items-center gap-1 text-[11px] font-medium text-warning">
                    <RefreshCw className="h-3 w-3" />{s.retryStatus}
                  </div>
                )}
                <div className="mt-3">
                  <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Scopes</div>
                  <div className="mt-1 flex flex-wrap gap-1">
                    {s.scopes.map((sc) => (
                      <Badge key={sc} variant="outline" className="h-5 border-border/70 bg-background text-[10px] font-mono font-normal text-muted-foreground">
                        {sc}
                      </Badge>
                    ))}
                  </div>
...
function SyncBadge({ status }: { status: SyncStatus }) {
  ...
  const m = map[status];
  return (
    <Badge variant="outline" className={cn("inline-flex h-5 items-center gap-1 border-transparent text-[10px] font-semibold ring-1 ring-inset", m.cls)}>
...
function HealthBadge({ status }: { status: "healthy" | "degraded" | "failed" }) {
  const map = {
    healthy: { cls: "bg-success/10 text-success ring-success/20", Icon: CheckCircle2, label: "Healthy" },
    degraded: { cls: "bg-warning/15 text-warning ring-warning/25", Icon: AlertTriangle, label: "Degraded" },
    failed: { cls: "bg-destructive/10 text-destructive ring-destructive/20", Icon: XCircle, label: "Failed" },
  } as const;
  const m = map[status];
  return (
...
function MiniStat({ label, value, tone }: { label: string; value: string; tone: "ok" | "warn" | "muted" }) {
