  ...
  return (
    ...
    >
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard label="Total Overhead" value={total} icon={Building2} tone="blue" delta={5.8} trend="up" />
        <KpiCard label="Fixed Overhead" value={fixed} icon={Wallet2} tone="violet" delta={2.1} trend="up" />
        <KpiCard label="Variable Overhead" value={variable} icon={TrendingUp} tone="cyan" delta={11.4} trend="up" />
        <KpiCard label="Overhead % Revenue" value={(total / revenue) * 100} icon={Percent} tone="mint" delta={-0.4} trend="down" format="percent" />
        <KpiCard label="Overhead / Employee" value={total / 22} icon={Users2} tone="violet" delta={1.2} trend="up" />
        <KpiCard label="Overhead / Client" value={total / 180} icon={Users2} tone="cyan" delta={-2.4} trend="down" />
        <KpiCard label="Budget Variance" value={-3_200} icon={Wallet2} tone="mint" delta={-3200} trend="down" compareLabel="under budget" />
        <KpiCard label="Forecasted Month-End" value={218_400} icon={TrendingUp} tone="blue" delta={1.6} trend="up" />
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        <Card className="border-border/70 p-4 xl:col-span-2">
          ...
          <p className="mt-0.5 text-[11px] text-muted-foreground">
            Text summary: overhead grew 18% while revenue grew 22% — ratio improved from 17.9% to 17.2%.
          </p>
          <div className="mt-3 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={OVERHEAD_TREND}>
                ...
                <Line dataKey="overhead" stroke="#f97316" strokeWidth={2} dot={false} name="Overhead" />
                <Line dataKey="budget" stroke="#22d3ee" strokeDasharray="4 4" strokeWidth={1.5} dot={false} name="Budget" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card className="border-border/70 p-4">
          ...
          <div className="mt-3 h-52">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={OVERHEAD_FIXED_VARIABLE} dataKey="value" innerRadius={45} outerRadius={80} paddingAngle={2}>
      ...
      <section>
        <Card className="border-border/70 p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] text-muted-foreground">{OVERHEAD_CATEGORIES.length} categories tracked</span>
          </div>
          <div className="mt-3 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={OVERHEAD_CATEGORIES} layout="vertical" margin={{ left: 24 }}>
                ...
                <YAxis type="category" dataKey="label" fontSize={11} tickLine={false} axisLine={false} width={140} />
                <Tooltip formatter={(v: number) => currency(v)} contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} />
                ...
                {OVERHEAD_CATEGORIES.map((c) => {
                  ...
                  return (
                    <tr key={c.key} className="border-t border-border/70">
                      ...
                      <td className="px-3 py-2 text-right font-tabular font-semibold">{currency(c.current)}</td>
                      <td className="px-3 py-2 text-right font-tabular text-muted-foreground">{currency(c.prior)}</td>
                      <td className="px-3 py-2 text-right font-tabular text-muted-foreground">{currency(c.budget)}</td>
                      <td className={cn("px-3 py-2 text-right font-tabular font-semibold", variance > 0 ? "text-destructive" : "text-success")}>
                        {variance > 0 ? "+" : ""}
                        {currency(variance)}
                      </td>
                      <td className="px-3 py-2 text-right font-tabular text-muted-foreground">{currency(c.ytd)}</td>
