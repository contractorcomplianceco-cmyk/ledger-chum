...
function InvoicesIndex() {
  ...
  const filtered = useMemo(() => {
    return INVOICES.filter((i) => {
      ...
      if (
        q &&
        !`${i.number} ${i.customerName} ${i.notes ?? ""}`.toLowerCase().includes(q.toLowerCase())
      )
        return false;
      return true;
    });
    ...
    <div className="space-y-5">
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard
          ...
          icon={FileText}
          tone="cyan"
          compareLabel="awaiting send"
        />
        <KpiCard
          label="Paid this period"
          value={kpis.paidThisPeriod}
          delta={18.2}
          trend="up"
          icon={CheckCheck}
          tone="mint"
          compareLabel={`${kpis.paidCount} invoices · avg ${kpis.avgPayDays}d to pay`}
        />
      </section>
