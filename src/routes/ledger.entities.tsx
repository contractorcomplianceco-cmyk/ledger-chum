...
function EntitiesPage() {
  ...
  const entities = useQuery({
    queryKey: ["legal-entities", orgId],
    queryFn: () => le({ data: { orgId: orgId! } }),
    enabled: !!orgId,
  });
  const txns = useQuery({
    queryKey: ["ic-txns", orgId],
    queryFn: () => ic({ data: { orgId: orgId!, status: "all" } }),
    enabled: !!orgId,
  });
  const balances = useQuery({
    queryKey: ["ic-balances", orgId],
    queryFn: () => bal({ data: { orgId: orgId! } }),
    enabled: !!orgId,
  });
          ...
          <div className="space-y-6">
            <Card className="p-0 overflow-hidden">
              <div className="border-b border-border p-3 text-sm font-semibold">
                Legal entities ({entities.data?.length ?? 0})
              </div>
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-left text-[11px] uppercase text-muted-foreground">
                  <tr>
                    ...
                    <th className="px-3 py-2">Consolidated</th>
                  ...
                  return (
                    <li key={i} className="flex justify-between border-b border-border/50 py-1">
                      ...
                      <span className="tabular-nums font-medium">
                        ${Number(b.net).toLocaleString()}
                      </span>
                    </li>
                  );
                })}
                {(balances.data?.length ?? 0) === 0 && (
                  <li className="text-muted-foreground">No open intercompany balances.</li>
                )}
              </ul>
            </Card>
            ...
            <Card className="p-0 overflow-hidden">
              <div className="border-b border-border p-3 text-sm font-semibold">
                Intercompany transactions ({txns.data?.length ?? 0})
              </div>
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-left text-[11px] uppercase text-muted-foreground">
                  <tr>
                ...
                <tbody>
                  {txns.data?.map((t) => (
                    <tr key={t.id} className="border-t border-border">
                      ...
                      <td className="px-3 py-2">
                        {entityMap.get(t.from_entity_id)?.code ?? t.from_entity_id.slice(0, 8)}
                      </td>
                      <td className="px-3 py-2">
                        {entityMap.get(t.to_entity_id)?.code ?? t.to_entity_id.slice(0, 8)}
                      </td>
