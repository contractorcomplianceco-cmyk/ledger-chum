...
export const Route = createFileRoute("/reports/cash-flow")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Operating cash flow via the indirect method, from posted journal lines.",
      },
      { property: "og:title", content: "Cash Flow — LedgerOS" },
      {
        property: "og:description",
        content: "Indirect cash flow: Net Income adjusted for working capital changes.",
      },
    ],
  }),
...
function Row({
  ...
  return (
    <div
      className={`flex justify-between py-2 ${strong ? "font-bold border-t-2 mt-2 pt-3" : "border-b"} ${muted ? "text-muted-foreground pl-6" : ""}`}
    >
      <span>{label}</span>
        ...
        <Card className="p-6">
          {!q.data ? (
            ...
            <div className="max-w-xl">
              ...
              <div className="pt-3 pb-1 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                Working capital adjustments
              </div>
              <Row
                label="− Increase in Accounts Receivable"
                amount={-q.data.adjustments.arIncrease}
                muted
              />
              <Row
                label="− Increase in Inventory"
                amount={-q.data.adjustments.inventoryIncrease}
                muted
              />
              <Row
                label="+ Increase in Accounts Payable"
                amount={q.data.adjustments.apIncrease}
                muted
              />
